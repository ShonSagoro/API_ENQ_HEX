...
export class MongoDBUserRepository implements UserInterface {
    ...
    async notifyUser(uuid: string, reservation: Reservation): Promise<void> {
        try {
            ...
            if (result) {
                await this.EmailService.sendEmail(
                    ...
                    "Reservation", 
                    "Your reservation has been created to hotel: " + reservation.getHotelUUID() + 
                    " with room number: " + reservation.getRoomNumber() + 
                    " from: " + reservation.getDateStart().toISOString() + 
                    " to: " + reservation.getDateEnd().toISOString()
                );            }       
        } catch (error) {
            console.error(error);
        }
